...
function countLength(field, text, max_length) {
  ...
  } else if (0 < text_length && text_length < max_length * 0.9) {
    target.setAttribute("class", "text-right text-success");
  } else if (max_length * 0.9 <= text_length && text_length <= max_length) {
    target.setAttribute("class", "text-right text-warning");
  } else {
    target.setAttribute("class", "text-right text-danger");
  }
  target.innerHTML = `文字数：${Number(text_length).toLocaleString()}/${Number(
    max_length
  ).toLocaleString()}`;
}
