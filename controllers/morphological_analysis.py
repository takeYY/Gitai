...
def render_result(basic_data: dict, description: dict,
                  option: dict = None, result: dict = None):
    return render_template('morphological_analysis/index.html',
                           basic_data=basic_data,
                           description=description,
                           option=option,
...
def show():
    ...
    # 結果のデータ群
    result = dict(df=mrph_df, file_name=csv_name,
                  dl_type='result', new_name=f'{mrph_type}による形態素解析結果')

    return render_result(basic_data,
                         description,
                         option,
                         result)
