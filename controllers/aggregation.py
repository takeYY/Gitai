...
aggregation_page = Blueprint("aggregation", __name__, url_prefix="/aggregation")


def render_data_selection(basic_data: dict, edogawa_data: dict, description: dict, input_data: dict = None):
    return render_template(
        "aggregation/data_selection.html",
...
def data_selection():
    ...
    # 形態素解析器の説明文
    description = dict(mrph=morphological_analysis_description(), csv_sample=csv_file_description())
    # 江戸川乱歩作品関連の情報
    edogawa_data = dict(name_file=get_novels_tuple(col1="name", col2="file_name"))

    session.clear()
    return render_data_selection(basic_data=basic_data, edogawa_data=edogawa_data, description=description)
...
def options():
    ...
    # 入力データの設定
    input_data = InputAggregation(request=request, session=session)
    ...
    if input_data.__dict__.get("errors"):
        return render_data_selection(
            basic_data=basic_data, edogawa_data=edogawa_data, description=description, input_data=input_data.__dict__
        )
    # 入力CSVの列名チェック
...
def result():
    ...
    # 利用者から送られてきた情報の取得
    option = OptionAggregation(request=request)
    # 品詞の含有数でソート
    input_data.hinshi_sort()
    ...
    if not option.hinshi_jpn:
        ...
        return render_options(basic_data=basic_data, input_data=input_data, option=option.__dict__)

    # データの集計処理
    try:
        result_df, file_name = create_aggregation(
            mrph_type=input_data.mrph_type, csv_name=input_data.csv_name, target_hinshi=option.hinshi_jpn
        )
    except Exception:
    ...
    # csvダウンロード設定
    dl_data = dict(file_name=file_name, dl_type="result", new_name=f"{input_data.name}の集計結果")
    # 結果情報を格納
    result_data = dict(file_name=file_name, html_file_name=file_name, df=result_df[:51])
    try:
