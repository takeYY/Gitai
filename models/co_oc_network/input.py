...
class InputCoOcNetwork:
    ...
    def __init__(self, request, session):
        form = request.form
        self.data_type: str = form.get("data_type", session.get("data_type"))
        self.mrph_type: str = form.get("mrph_type", session.get("mrph_type")) if self.data_type == "edogawa" else ""
        self.set_input_name_csv(request, session)
        # カテゴリごとの表示有無
        self.is_used_category: int = int(form.get("is_used_category", session.get("is_used_category", 0)))
    ...
    def set_input_name_csv(self, request, session):
        ...
        if session.get("input_name"):
            ...
            (self.name, self.csv_name, self.errors) = get_csv_filename(request)
        else:
    ...
    def set_hinshi(self):
        self.hinshi: dict = get_unique_hinshi_dict(self.mrph_type, self.csv_name)

    def set_category_list(self):
        self.category_list: list = get_category_list(self.csv_name) if self.is_used_category == 1 else []
    ...
    def get_table_dict(self):
        return {
            "入力データタイプ": "江戸川乱歩作品" if self.data_type == "edogawa" else "CSVデータ",
            "入力データ名": self.name,
            "章ごとのカテゴリ分割": "する" if self.is_used_category == 1 else "しない",
            "形態素解析器": "MeCab"
            if self.data_type == "edogawa" and self.mrph_type == "mecab"
            else "Jumanpp"
            if self.data_type == "edogawa" and self.mrph_type == "juman"
            else "",
        }
    ...
    def hinshi_sort(self):
        self.hinshi = dict(sorted(self.hinshi.items(), key=lambda x: x[1], reverse=True))
