...
def get_morphological_analysis_dict():
    return dict(
        ...
        NEologd="NEologd",
        Jumanpp="Jumanpp",
        output_sample_mecab="MeCab（NEologd）の出力例",
        output_sample_jumanpp="Jumanppの出力例",
    )


@timeout_decorator.timeout(29, use_signals=False)
def get_mecab_mrph(text):
    running_mode = os.environ.get("MODE")
    if running_mode == "PROD":
        tagger = MeCab.Tagger(os.environ.get("PROD_MECAB_DICT_TARGET", ""))
    else:
        tagger = MeCab.Tagger(os.environ.get("M1MAC_MECAB_DICT_TARGET", ""))
    # MeCabの分類リスト取得
