...
def get_co_oc_strength_dict():
    return dict(
        jaccard="Jaccard係数（0.0〜1.0）",
        dice="Dice係数（0.0〜1.0）",
        simpson="Simpson係数（0.0〜1.0）",
        frequency="共起頻度（0〜∞）",
        pmi="相互情報量（-∞〜∞）",
    )
...
def get_edogawa_merge_with_rows_df():
    ...
    for idx, rows in edogawa.iterrows():
        row_name = rows["name"]
        df = edogawa_merge.query(f" name == {row_name} ")
...
def mecab_divide_dict():
    return {
        "surface": "表層形",
        "hinshi": "品詞",
        "hinshi_detail_1": "品詞細分類(1)",
        "hinshi_detail_2": "品詞細分類(2)",
        "hinshi_detail_3": "品詞細分類(3)",
        "katsuyou_type": "活用型",
        "katsuyou_form": "活用形",
        "genkei": "原形",
        "yomi": "読み",
...
def get_datetime_now(hours: int = 9):
    return datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=hours))).strftime("%Y_%m_%d_%H_%M_%S%f")


def create_category_df(df: pd.DataFrame):
    try:
        big_captions = [big if big == big else "" for big in df["大見出し"].tolist()]
        middle_captions = [middle if middle == middle else "" for middle in df["中見出し"].tolist()]
        small_captions = [small if small == small else "" for small in df["小見出し"].tolist()]
        categories = []
...
def create_random_string(n):
    rand_str_list = [random.choice(string.ascii_letters + string.digits) for i in range(n)]
