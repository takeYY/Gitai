...
def get_unique_hinshi_dict(mrph_type, csv_filename):
    ...
    return df[limited_columns].groupby("品詞").agg("count").to_dict().get(basic_type)
    ...
    for idx, hinshi in enumerate(agg_df["品詞"].unique().tolist()):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=tmp_df["出現頻度"],
                text=tmp_df["単語"],
                textposition=[text_positions[i % len(text_positions)] for i in range(len(tmp_df))],
                customdata=[
...
def create_aggregation(mrph_type, csv_name, target_hinshi=["名詞"]):
    ...
    mrph_columns = get_mrph_column(mrph_type) if mrph_type else get_mrph_column(get_csv_mrph_type(df))
    # 表層形 or 見出し
    basic_type = mrph_columns[0]
    # 原形 or 原型
    ...
    # 集計データの出力
    agg_df.to_csv(f"tmp/{html_random_name}.csv", index=False, encoding="utf_8_sig")
    # 集計グラフのhtml出力
