...
def modify_df_with_synonym(tmp_df: pd.DataFrame, synonym: str) -> pd.DataFrame:
    ...
    for syno_dict in synonyms:
        df_match = tmp_df.query(" 原形 in @syno_dict.keys() or 表層形 in @syno_dict.keys() ", engine="python")
        ...
        while True:
            ...
            for key in syno_dict.keys():
                tgt_m2 = tgt - 2
                tgt_p2 = tgt + 2
                t_df = tmp_df[tgt_m2:tgt_p2].query(" 原形==@key or 表層形==@key ", engine="python")
                if t_df.empty:
                    try:
                        target.remove(tgt)
                    except Exception:
        ...
        for tgt in target:
            tgt_m2 = tgt - 2
            tgt_p2 = tgt + 2
            new_df = tmp_df[tgt_m2:tgt_p2].replace({"原形": syno_dict})
            new_df.at[tgt, "品詞"] = "固有名詞"
            tmp_df.iloc[tgt_m2:tgt_p2] = new_df
...
def create_co_oc_df(keywords, sentence_combinations, all_data_size, category=""):
    ...
    if not category:
        columns = ["単語a", "単語aの品詞", "単語b", "単語bの品詞", "intersection_count"]
        (on1_list, on2_list) = (["単語a", "単語aの品詞"], ["単語b", "単語bの品詞"])
    else:
        columns = ["単語a", "単語aの品詞", "単語b", "単語bの品詞", "category", "intersection_count"]
        (on1_list, on2_list) = (["単語a", "単語aの品詞", "category"], ["単語b", "単語bの品詞", "category"])

    word_associates = []
    for key, value in combi_count.items():
        if not category:
            word_associates.append([key[0][0], key[0][1], key[1][0], key[1][1], value])
        else:
            word_associates.append([key[0][0], key[0][1], key[1][0], key[1][1], category, value])

    word_associates = pd.DataFrame(word_associates, columns=columns)

    word_count = collections.Counter(sum(keywords, []))
    if not category:
        word_count = [[key[0], key[1], value] for key, value in word_count.items()]
        word_count = pd.DataFrame(word_count, columns=["word", "word_type", "count"])
    else:
        word_count = [[key[0], key[1], category, value] for key, value in word_count.items()]
        word_count = pd.DataFrame(word_count, columns=["word", "word_type", "category", "count"])

    word_associates = (
        pd.merge(
            word_associates, word_count.rename(columns={"word": "単語a", "word_type": "単語aの品詞"}), on=on1_list, how="left"
        )
        .rename(columns={"count": "count1"})
        .merge(word_count.rename(columns={"word": "単語b", "word_type": "単語bの品詞"}), on=on2_list, how="left")
        .rename(columns={"count": "count2"})
        .assign(union_count=lambda x: x.count1 + x.count2 - x.intersection_count)
        .reset_index(drop=True)
    )

    # jaccard係数の作成
    word_associates["jaccard_coef"] = word_associates["intersection_count"] / word_associates["union_count"]
    # dice係数の作成
    word_associates["dice_coef"] = (
        2 * word_associates["intersection_count"] / (word_associates["count1"] + word_associates["count2"])
    )
    # simpson係数作成
    simpson_coef = [
        intersection / min(count1, count2)
        for intersection, count1, count2 in zip(
            word_associates["intersection_count"].tolist(),
    ...
    # 相互情報量作成
    pmi_coef = [
        math.log2((intersection * all_data_size) / (count1 * count2))
        for intersection, count1, count2 in zip(
            word_associates["intersection_count"], word_associates["count1"], word_associates["count2"]
        )
    ]
...
def kyoki_word_network(target_num=250, file_name="3742_9_3_11_02", target_coef="共起頻度"):
    ...
    for node in got_net.nodes:
        node["title"] += "<br>共起:<br>" + "<br>".join(neighbor_map[node["id"]])
...
def create_2D_network_figure(df, target_num, fig, target_coef):
    ...
    for e in G.edges():
        ...
        if weight < edge_group[0]:
            edges = append_edge_dictionary(edges, 1, x=x_list, y=y_list, z=None, weight=weight)
        elif edge_group[0] <= weight < edge_group[1]:
            edges = append_edge_dictionary(edges, 2, x=x_list, y=y_list, z=None, weight=weight)
        elif edge_group[1] <= weight < edge_group[2]:
            edges = append_edge_dictionary(edges, 3, x=x_list, y=y_list, z=None, weight=weight)
        else:
            edges = append_edge_dictionary(edges, 4, x=x_list, y=y_list, z=None, weight=weight)
    ...
    for n in G.nodes():
        ...
        if frequency < node_group[0]:
            nodes = append_node_dictionary(nodes, 1, x, y, None, frequency, text, info)
        elif node_group[0] <= frequency < node_group[1]:
            nodes = append_node_dictionary(nodes, 2, x, y, None, frequency, text, info)
        elif node_group[1] <= frequency < node_group[2]:
            nodes = append_node_dictionary(nodes, 3, x, y, None, frequency, text, info)
        else:
            nodes = append_node_dictionary(nodes, 4, x, y, None, frequency, text, info)
    ...
    for idx in range(len(marker_size)):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(size=marker_size[idx], line=dict(width=2), opacity=0.5, colorscale="jet"),
...
def create_network(
    ...
    data_type: str, default='edogawa'
        入力データの種類（江戸川乱歩作品: 'edogawa', csv: 'csv'）
    ...
    if data_type == "edogawa":
        ...
        # カテゴリーごとの分析をする かつ 3D表示ならば
            ...
            if mrph_type == "juman":
                # jumanppにより形態素解析したDF取得
                df = get_jumanpp_df(file_name).rename(columns={"章ラベル": "カテゴリー"})
            else:
                # MeCabにより形態素解析されたDF取得
                df = get_mecab_df(file_name)
            if selected_category:
                # 見出しからカテゴリー作成後選択したカテゴリーに絞る
                df = create_category_df(df).query(" カテゴリー in @selected_category ")
    else:
        df = pd.read_csv(f"tmp/{file_name}.csv")
    # カラム名を統一
    df = df.rename(columns={"見出し": "表層形", "原型": "原形"})
    ...
    if used_category == 1 and is_used_3d:
        for cat in df["カテゴリー"].unique():
            keywords_dict[cat] = create_keywords(df.query(" カテゴリー==@cat "), remove_words, target_words, target_hinshi)
            sentence_combinations_dict[cat] = create_sentence_combinations(keywords_dict[cat], remove_combi)
            all_data_size = len(sum(keywords_dict[cat], []))
            tmp_df = create_co_oc_df(keywords_dict[cat], sentence_combinations_dict[cat], all_data_size, category=cat)
            co_oc_df = pd.concat([co_oc_df, tmp_df]).reset_index(drop=True)
    else:
        key_name = "all"
        keywords_dict[key_name] = create_keywords(df, remove_words, target_words, target_hinshi)
        sentence_combinations_dict[key_name] = create_sentence_combinations(keywords_dict[key_name], remove_combi)
        all_data_size = len(sum(keywords_dict[key_name], []))
        co_oc_df = create_co_oc_df(keywords_dict[key_name], sentence_combinations_dict[key_name], all_data_size)
    # カラム名の変更
    co_oc_df = co_oc_df.rename(
        columns={
            "category": "カテゴリー",
            "count1": "単語aの出現頻度",
            "count2": "単語bの出現頻度",
    ...
    # 共起頻度の最小値以上に制限
    co_oc_df = co_oc_df.query(" @co_oc_freq_min <= 共起頻度 ").reset_index(drop=True)
    # 強制抽出語が含まれるレコードのみ抽出
    if target_words:
        new_co_oc_df = pd.DataFrame()
        for tw in target_words:
            new_co_oc_df = pd.concat([new_co_oc_df, co_oc_df.query(" @tw in 単語a or @tw in 単語b ")])
        co_oc_df = (
            new_co_oc_df.sort_values(target_coef, ascending=False)
            .drop_duplicates(subset=["単語a", "単語b"])
            .reset_index(drop=True)
        )
    # 表示する共起強度を制限
    co_oc_df = co_oc_df[co_oc_df[target_coef] <= strength_max].reset_index(drop=True)
    # target_coefでソート
    co_oc_df = co_oc_df.sort_values(target_coef, ascending=False).reset_index(drop=True)
    # 所定の構造でCSVファイルに出力
    file_random_name = create_random_string(32)
    co_oc_df.to_csv(f"tmp/{file_random_name}.csv", index=False, encoding="utf_8_sig")
    # 小数点以下2桁で丸める
    ...
    try:
        ...
        if dimension == 4:
            create_2D_detail_network(co_oc_df, target_num, target_coef, file_random_name)
        # 2Dの共起ネットワークHTML作成（シンプルバージョン）
        elif not is_used_3d:
            got_net = kyoki_word_network(target_num=target_num, file_name=file_random_name, target_coef=target_coef)
